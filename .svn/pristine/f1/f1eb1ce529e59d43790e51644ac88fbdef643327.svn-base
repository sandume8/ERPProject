...
<script src='${CTX_PATH}/js/sweetalert/sweetalert.min.js'></script>
    ...
    var calendar;
			    ...
			    success: function (res) {
			      if (res.length) {
			        ...
			        for (var i =0; i<res.length;i++){
			        		text+= (i+1)+'. '+ res[i].deli_company+'의 '+res[i].pro_name +'\n'
			        		text+= '[진행상황] : ' + res[i].detail_name+'\n\n'
			        }
			        swal(text);
			      } else {
			        // 이벤트 목록이 없을 때
     ...
     // 일정 조회
  	...
  	function fCalendar() {
  		...
  		callAjax("/dashboard/calendarList.do", "post", "json", true, param, resultCallback);
  	}
	...
	function fRedrawCalendar(data) {
		for (var i=0;i<data.length;i++){
			calendar.addEvent( {
				'title':''+data[i].deli_company+'의 '+data[i].pro_name+'', 
				'start':''+data[i].purchase_date+'', 
				});
		}
	}
