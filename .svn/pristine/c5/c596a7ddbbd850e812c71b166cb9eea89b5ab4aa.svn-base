...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#to_date_modal").change(function() {
			...
			if ($("#lev_type_modal").val() == '1') {
				if ($("#from_date_modal").val() != $("#to_date_modal").val()) {
					swal("반차 선택시 날짜를 동일하게 선택해야 합니다.").then(function(){
						$("#to_date_modal").val(today);
					});
				}
			 }
		});
		...
		/*
		$("#lev_type_modal").change(function() {
			if ($("#lev_type_modal").val() == '1') {
				var from_date_modal = $("#from_date_modal").val();
				alert("시작 날짜는 :"+from_date_modal);
				$("#to_date_modal").val();
				if($("#from_date_modal").val() != $("#to_date_modal").val()){
					swal("반차 선택시 날짜를 동일하게 선택해야 합니다.")
				}
			}
		}); */
	}); 
						...
						<thead class="middle">
							<tr>
								...
								<td scope="col" class="center" style="width: 200px; height: 30px; padding: 0px; text-align: center; display: block; margin: 0 auto;">
									<select id="lev_type_modal" class="center" style="width: 200px; height: 30px; padding: 0px; text-align: center; display: block; margin: 4px 0px 4px 0px;">
										<option value="3">연차</option>
										<option value="2">월차</option>
										<option value="1">반차</option>
									</select> 
