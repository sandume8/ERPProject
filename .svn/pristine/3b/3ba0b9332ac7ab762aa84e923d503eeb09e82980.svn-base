...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
							...
							<c:if test="${totalCntBmDv eq 0 }">
								<tr>
									<td colspan="12">데이터가 존재하지 않습니다.</td>
								</tr>
							</c:if>
							
							<c:if test="${totalCntBmDv > 0 }">
								<c:set var="nRow" value="${pageSize*(currentPageBmDv-1)}" />
								<c:forEach items="${listBmDvModel}" var="list">
									<tr>
										<td><a href="javascript:fPopModalBmDv('${list.dv_no}')">${list.dv_no}</a></td>				<!-- 결의번호 -->
							...
							<input type="hidden" id="totalCntBmDv" name="totalCntBmDv" values="${totalCntBmDv}"/>